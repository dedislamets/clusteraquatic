		...
		setInterval(function(){
		    ...
		    $.ajax({
		        ...
		        success: function(response) {
					var thead_data = response.keys;
					var tbody = response.data;
					var iuranTable = '';
					iuranTable += '<table id="datatable-fixed-header" class="table table-striped table-bordered">';
						iuranTable += '<thead>';
							iuranTable += '<tr>';
								$.each( thead_data, function( key_thead, val_thead ) {
									thead_ = val_thead.replace('_', ' ');
									thead_upper = thead_.substring(0, 1).toUpperCase() + thead_.substring(1);
									iuranTable += '<th>'+thead_upper+'</th>';
								});
							iuranTable += '</tr>';
						iuranTable += '</thead>';
						iuranTable += '<tbody>';
		...
		setInterval(function(){
		    ...
		    $.ajax({
				...
				success: function(data) {
					var legend_chart = data.byStatus.legend;
					var data_chart = data.byStatus.data;
					var chartIuranpie = echarts.init(document.getElementById('iuranChart'), themeChart);
					chartIuranpie.setOption({
						tooltip: {
							trigger: 'item',
							formatter: "{a} <br/>{b} : {c} ({d}%)"
						},
	...
	function dataRT()
	{
		$.ajax({
	        ...
	        success: function(dataRT) {
	        	var valueRT = dataRT.data;
	        	var idRT = dataRT.id;
	        	var rtData = '';
		        var i;
		        var ii;
				for (i = 0; i < valueRT.length; i++) {
